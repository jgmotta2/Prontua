...
// Placeholders das demais features — serão migradas do protótipo em
// rodadas seguintes (pacientes, agenda, financeiro, configurações).
const PlaceholderPage = ({ title }: { title: string }) => (
  <div className="p-8">
    <h1 className="font-display text-3xl font-semibold text-ink">{title}</h1>
    <p className="text-muted mt-2">Tela em construção — migrar do protótipo.</p>
  </div>
);
...
export function AppRoutes() {
  return (
    <Routes>
      ...
      <Route path="/pacientes"     element={<Private><PlaceholderPage title="Pacientes" /></Private>} />
      <Route path="/pacientes/:id" element={<Private><PlaceholderPage title="Detalhe do paciente" /></Private>} />
      <Route path="/agenda"        element={<Private><PlaceholderPage title="Agenda" /></Private>} />
      <Route path="/financeiro"    element={<Private><PlaceholderPage title="Financeiro" /></Private>} />
