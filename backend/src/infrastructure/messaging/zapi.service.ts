...
export class ZApiService {
  ...
  /**
   * Envia um documento binário (PDF) via Z-API.
   * Não passa por assertClinicalSafe — o documento é intencionalmente clínico
   * e compartilhado de forma controlada (LGPD Art. 7º, VI — exercício regular de direito).
   */
  async sendClinicalDocument(
    tenantId: string,
    phoneE164: string,
    ...
    const logEntry = await prisma.whatsappLog.create({
      data: { tenantId, patientId, kind: 'CLINICAL_EXPORT' as any, template: 'prontuario_pdf', status: 'QUEUED' },
    });

    try {
      const ok = await this.callZApiDocument(phoneE164, base64Content, fileName);
  ...
  private async callZApiDocument(phone: string, base64: string, fileName: string): Promise<{ id: string }> {
    if (!env.ZAPI_INSTANCE_ID || !env.ZAPI_TOKEN) {
      logger.info({ phone, fileName }, 'zapi_simulated_document_send');
      return { id: `sim_${Date.now()}` };
    }

    const res = await fetch(`${this.baseUrl}/send-document/base64`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Client-Token': env.ZAPI_CLIENT_TOKEN!,
      },
      body: JSON.stringify({ phone, document: base64, extension: 'pdf', fileName }),
    });
    ...
    const json = (await res.json()) as { zaapId?: string; messageId?: string; id?: string };
    return { id: json.zaapId ?? json.messageId ?? json.id ?? 'unknown' };
  }
